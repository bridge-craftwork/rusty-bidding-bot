...
pub fn check(module: &Module) -> Vec<Diagnostic> {
    ...
    let mut report = |line: usize, message: String| {
        diags.push(Diagnostic { file: module.file.clone(), line, col: 0, message })
    };
    let mut check_path = |line: usize, path: &str, value: Option<&Literal>| match registry().get(path) {
        None => report(line, format!("unknown card field `{path}` (see crates/bridge-card/data/fields.toml)")),
        Some(field) => {
            if field.path != path {
                report(line, format!("`{path}` is an old name; use `{}`", field.path));
            }
            if let Some(v) = value {
